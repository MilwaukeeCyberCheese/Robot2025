...
public class DriveCommand extends Command {
   ...
   *
   * @param driveSubsystem subsystem for driving the robot {@link
   *     frc.robot.subsystems.DriveSubsystem link}
   ...
   * @param fieldRelative whether commands are relative to the field or the robot, true is relative
   *     to the field
  ...
  public DriveCommand(
    ...
    if (slow.getAsBoolean()) {
      ...
      m_rotSpeed =
          () ->
              rotSpeed.getAsDouble()
                  * Constants.DriveConstants.kSlowModifier
                  * Constants.DriveConstants.kTurningSpeedModifier;
    } else {
      ...
      m_rotSpeed =
          () ->
              rotSpeed.getAsDouble()
                  * MathUtil.clamp(throttle.getAsDouble(), 0.1, 1.0)
                  * Constants.DriveConstants.kTurningSpeedModifier;
    }
  ...
  public void end(boolean interrupted) {
    m_driveSubsystem.drive(
        0.0, 0.0, 0.0, m_fieldRelative.getAsBoolean(), m_rateLimit.getAsBoolean());
  }
}
